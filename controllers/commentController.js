...
const allComments = async (req, res, next) => {
    try {
        ...
        next(craeteError('Comments could not be fetched',500));
    }
...
const updateCommentStatus = async (req, res, next) => {
    try {
        ...
        if(!comment){
            return next(craeteError('Comment not found',404));
        }
        // res.redirect('/admin/comments');
        res.json({ success: true });
    } catch (error) {
        next(craeteError('Comment status could not be updated',500));
    }
 };
...
const deleteComment = async (req, res, next) => {
    try {
        ...
        next(craeteError('Comment status could not be deleted',500));
    }
 };
