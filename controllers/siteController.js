...
const craeteError = require('../utils/error-message');
...
const articlesByCategories = async (req, res, next) => {
    const category = await categoryModel.findOne({ slug: req.params.name });

    if (!category) {
        return next(craeteError('Category not found', 404));
    }
...
const singleArticle = async (req, res, next) => {
    ...
    if(!singleNews) {
        return next(craeteError('Article not found', 404));
    }
...
const author = async (req, res, next) => {
    ...
    if (!author) {
        ...
        return next(craeteError('Author not found', 404));
    }
...
const addComment = async (req, res, next) => {
    try {
        ...
        return next(craeteError('Comment could not be added', 500));
    }
    
 }

const testing = async (req, res, next) => {
    const news = await settingModel.findOne();
    res.json( news );
}
