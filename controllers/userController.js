...
const adminLogin = async (req, res, next) => {
    ...
    try {
        ...
        if(!user) {
            return next(craeteError('Invalid username or password', 401));
...
const saveSettings = async (req, res, next) => {
    ...
    try {
        ...
        if(!setting){
            return next(new Error('Setting not found'));
        }
        ...
        if(website_logo){
            if(setting.website_logo){
                const logoPath = `
                ${process.cwd()}/public/uploads/${setting.website_logo}`;
...
const updateUserPage = async (req, res, next) => {
    ...
    try {
        ...
        if(!user) {
            return next(craeteError('User not found', 404));
        }
